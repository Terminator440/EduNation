...
CREATE POLICY "Teachers can insert grades (teacher-subject access)" ON public.grades
  FOR INSERT
  WITH CHECK (
    (select auth.uid()) IN (SELECT teacher_id FROM public.subjects WHERE id = subject_id)
  );
...
-- SELECT: Students, their teachers, and parents can view grades
-- Rule: (select auth.uid()) = student_id OR (select auth.uid()) = teacher_id OR (select auth.uid()) IN (SELECT parent_user_id FROM parent_student_relations WHERE student_id = grades.student_id)
-- Implementation: 
...
CREATE POLICY "Students teachers and parents can view grades" ON public.grades
  FOR SELECT
  USING (
    -- Student can view their own grades ((select auth.uid()) matches students.user_id where students.id = student_id)
    (select auth.uid()) IN (SELECT user_id FROM public.students WHERE id = student_id AND user_id IS NOT NULL)
    OR
    -- Teacher can view grades ((select auth.uid()) = teacher_id in grades table)
    ((select auth.uid()) = teacher_id AND teacher_id IS NOT NULL)
    OR
    -- Teacher assigned to the subject can view grades ((select auth.uid()) matches subjects.teacher_id)
    (select auth.uid()) IN (SELECT teacher_id FROM public.subjects WHERE id = subject_id AND teacher_id IS NOT NULL)
    OR
