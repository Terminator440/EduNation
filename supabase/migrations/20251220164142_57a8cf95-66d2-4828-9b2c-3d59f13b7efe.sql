...
WITH CHECK (
  ...
  (has_role((select auth.uid()), 'homeroom_teacher'::public.app_role) OR has_role((select auth.uid()), 'secretariat'::public.app_role) OR has_role((select auth.uid()), 'director'::public.app_role))
);
...
ON public.classes
FOR ALL
USING (has_role((select auth.uid()), 'secretariat'::public.app_role) OR has_role((select auth.uid()), 'director'::public.app_role));
